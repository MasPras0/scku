...
# Tentukan versi yang diinginkan
desired_version="v24.11.11"

# Unduh dan jalankan installer dengan versi spesifik
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $desired_version
...
cat /etc/xray/xray.key /etc/xray/xray.crt | tee /etc/haproxy/ht.pem
